...
class InferenceService:
    ...
    def _apply_verification_feedback(
        ...
        *,
        ...
        """Apply verification outcomes to peer health scores.

        Rewards peers whose output matched the winning result and penalises
        those that diverged via reputation penalties.
    ...
    def infer(
        ...
        *,
        ...
        Raises:
            RuntimeError: If no peers found.
        ...
        if _use_specpipe:
            ...
            def _stage_fn(peer, activation, prompt="", stage_index=0,
                ...
                # inference hot path — it caused the relay to drop the
                # existing circuit (relay v2 max_circuits_per_peer=1)
                # when the forced re-dial created a new one. See the
                # equivalent comment in peer/server.py _push_to_next_hop.
            ...
            if _ar_tokenizer is not None:
                ...
                if _use_ring:
                    ...
                    for _ring_step in range(_ar_target_tokens):
                        try:
                            # Each token requires a full relay round-trip (~30-45s).
                            _ring_timeout = 120.0
