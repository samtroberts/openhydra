...
pytestmark = pytest.mark.skipif(
    ...
    reason="set OPENHYDRA_RUN_REAL_TENSOR_TEST=1 to run real PyTorch speculative decode validation",
)
...
def test_real_speculative_decoding_accepts_multiple_tokens_per_verification_round(tmp_path):
    ...
    try:
        started = [
            _start_pytorch_peer("peer-spec-a", 0, model_name),
            _start_pytorch_peer("peer-spec-b", 1, model_name),
        ]
    except Exception as exc:
        pytest.skip(f"unable to initialize pytorch speculative peers for model '{model_name}': {exc}")

    servers = [item[0] for item in started]
    ports = [item[1] for item in started]

    peers_config = tmp_path / "peers.spec.json"
    ...
    try:
        ...
        speculative = streaming["speculative"]

        assert streaming["mode"] == "pytorch_speculative_decode"
        assert streaming["speculative_enabled"] is True
        assert streaming["pytorch"]["enabled"] is True
        assert speculative["enabled"] is True
        assert int(speculative["rounds"]) >= 1
        assert int(speculative["accepted_tokens"]) >= 2
        assert int(speculative["verified_tokens"]) >= int(speculative["accepted_tokens"])
        assert text.strip() != ""
